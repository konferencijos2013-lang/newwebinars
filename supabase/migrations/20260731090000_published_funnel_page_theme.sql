-- Make each published funnel page theme available to the public renderer.
...
returns table (funnel_name text, page_name text, theme jsonb, blocks jsonb)
language sql
stable
security definer
...
as $$
  ...
  group by f.id, f.name, p.id, p.name, p.theme
$$;
